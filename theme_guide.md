...
- **实现原理**: 这类主题通过 JS 向主系统"声明"自己是一个覆盖层，主系统收到通知后，会自动将该主题的容器设置为全屏固定定位并启用鼠标穿透。
- **实现步骤**:

  **A. 尺寸配置 (JSON)**: 将所有支持位置的 `width` 和 `height` 都设置为 `100vw` 和 `100vh`（或 `100dvh`），让容器铺满屏幕。
  ...
  > **`vh` vs `dvh` 如何选择？**
  > - `100vh`：键盘弹出时容器大小不变，底部会被键盘遮挡（适合纯装饰性特效，如花瓣雨）
  > - `100dvh`：键盘弹出时容器会缩小，内容可能被压扁（适合需要始终完全可见的内容）
  ...
  **B. JavaScript 通知**: 在你的主题 JS 代码初始化时（例如 `DOMContentLoaded` 事件中），调用 `ThemeUtils.sendMessage` 发送通知。
  ...
  > **系统会自动处理**: 收到此消息后，系统会强制将容器设置为 `position: fixed; top: 0; left: 0; right: 0; bottom: 0`，确保主题在任何情况下都能正确铺满屏幕，不受键盘弹出等因素影响。
...
你**不需要**在主题里写任何拖动相关的代码。

主系统会自动给所有以 `播放器` 或 `Player` 开头的主题注入统一的拖动逻辑。用户按住播放器空白区域即可拖动，无论"位置锁定"开关是否打开。

**对开发者的影响**：

- ✅ 你**不需要**手写 `pointerdown/pointermove/pointerup` 处理
- ✅ 你**不需要**给主题加拖动覆盖层
- ✅ 用户的"锁定位置"复选框对播放器**已无作用**
- ⚠️ 如果你的主题里有**自定义可点击元素**（比如自定义滑块），但又不希望它触发拖动，请给那个元素加 `data-no-drag` 属性
...
> **关于自动隐藏功能**：用户开启"自动隐藏"后，主系统会用 `visibility + opacity` 控制播放器显隐，**不会销毁 iframe**。所以播放器隐藏期间你的 JS 仍在运行，音乐继续播、进度不丢，无需特殊处理。

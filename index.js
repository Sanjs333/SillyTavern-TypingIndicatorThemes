...
async function createUnifiedIframe(theme, indicatorElement, characterName) {
  ...
  let playlist = null;
  if (theme.name.startsWith("播放器-") || theme.name.startsWith("Player - ")) {
  ...
  const render = (preserveDrawerState = false) => {
    ...
    <div class="inline-drawer-content" style="display: none; flex-direction: column; gap: 15px;">
        <div style="display: flex; flex-direction: column; align-items: flex-end; gap: 8px;">
            ...
            <button id="ti_restore_defaults_btn"
                    title="${t`Restore Built-in Items`}"
                    style="background: none; border: none; padding: 0; cursor: pointer; color: var(--text-color-secondary); font-size: 0.9em; display: inline-flex; align-items: center; gap: 5px;">
                <i class="fa-solid fa-arrows-rotate"></i>
                <span>${t`Restore Built-in Items`}</span>
            </button>
        </div>
...
`;
    ...
    function updateRenameButtonState() {
      ...
      const isThemeBuiltIn =
        selectedThemeOption && selectedThemeOption.dataset.isbuiltin === "true";
    ...
    section.querySelector("#ti_preset_save").addEventListener("click", () => {
      ...
      if (p) {
        ...
        saveSettingsDebounced();
        toastr.success(
          t`Preset "{{presetName}}" saved.`.replace("{{presetName}}", p.name)
        );
      }
    });
    ...
    tiThemeSelect.addEventListener("change", () => {
      ...
      if (getSettings().persistentMode) {
        console.log("[TypingIndicator] Retheming persistent indicator...");
      ...
      if (selectedTheme && !selectedTheme.useIframe) {
        ...
        if (
          ...
          applyLink.on("click", (e) => {
            ...
            saveSettingsDebounced();
            suggestionContainer.remove();
            toastr.success(`已应用内容方案: "${recommendedPreset.name}"`);
          });
          suggestionContainer.html(message).append(applyLink);
    ...
    section.querySelector("#ti_theme_save").addEventListener("click", () => {
      ...
      const isModeIframe = section.querySelector("#theme_mode_iframe").checked;
      themeToSave.useIframe = isModeIframe;

      if (isModeIframe) {
        if (!validateSizesJson()) {
          toastr.error("尺寸配置 (Sizes) 的JSON格式有误，请修正！", "保存失败");
          return;
        }
        ...
        if (sizesText) {
          try {
            ...
            toastr.error("无法解析尺寸配置，请检查格式。", "保存失败");
            return;
          }
        } else {
    ...
    section.querySelector("#preview_theme").addEventListener("click", () => {
      ...
      if (previewTheme.useIframe) {
        ...
        createUnifiedIframe(previewTheme, previewIndicator, previewCharName);
      } else {
        ...
        const tiPresetSelect = section.querySelector("#ti_preset_select");
        const textPreset =
          settings.textPresets.find((p) => p.id === tiPresetSelect.value) ||
          settings.textPresets[0];

        let htmlContent = (textPreset.text || "{{char}} is typing...")
    ...
    lockCheckbox.addEventListener("change", (e) => {
      ...
      if (indicator && settings.position === "draggable") {
        const oldHandle = indicator.querySelector('div[style*="cursor: move"]');
    ...
    const persistentModeCheckbox = section.querySelector("#ti_persistent_mode");
    ...
    const downloadGuideButton = section.querySelector("#ti_download_guide_btn");
